...
- **Run `node scripts/check-consistency.mjs` before claiming done.** It enforces: no orphan FKs, period-log↔daily-attendance sync, class coverage, notification wiring, no raw `<textarea>`, `bare` in table cells, VN name sorting, no raw `toLocale*`, nav naming, no dead buttons. Adding a shared-fact write path or a new people list → extend the script, do not bypass it.
